...
public class CatmintException extends RuntimeException {
    ...
    public CatmintException(final Class<?> clazz, final Exception ex) {
        super( String.format( "Can not find public default constructor for SPI class `%s`", clazz.getName() ), ex );
    }
}
