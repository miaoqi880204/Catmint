...
public final class MySQLAuthenticator {
    ...
    public boolean auth(final ChannelHandlerContext context, final PacketPayload payload) {
        ...
        if (!Strings.isNullOrEmpty( response41.getDatabase() ) && !CatmintConfigUtils.getSchemaDatabaseNodeToString( response41.getUsername() ).contains( response41.getDatabase() )) {
    ...
    private Optional<MySQLServerErrorCode> login(final MySQLHandshakeResponse41Packet response41) {
        Optional<ProxyUser> user = CatmintConfigUtils.getServerConfUser( response41.getUsername() );
