package org.catmint.proxy.frontend.auth.mysql;
...
import org.catmint.proxy.packet.mysql.*;
 ...
 */
@Getter
public final class MySQLAuthenticator implements AuthenticationEngine {
    ...
    public boolean auth(final ChannelHandlerContext context, final PacketPayload payload) {
        ...
        if (!Strings.isNullOrEmpty( response41.getDatabase() )) {
            if (!CatmintConfigUtils.getDatabases( response41.getUsername() ).contains( response41.getDatabase() )) {
                ...
                context.writeAndFlush( getMySQLErrPacket( MySQLServerErrorCode.ER_DBACCESS_DENIED_ERROR, context, response41 ) );
            }
        }
    ...
    private Optional<MySQLServerErrorCode> login(final MySQLHandshakeResponse41Packet response41) {
        Optional<ProxyUser> user = CatmintConfigUtils.getProxyUser( response41.getUsername() );
